...
bool is_path_safe(const string &root_dir, const string &test_file) {
  ...
  // TODO: implment
  char directory[PATH_MAX], file[PATH_MAX];
  char* directory_path = realpath(root_dir.c_str(), directory);
  char* file_path = realpath(test_file.c_str(), file);
  if (!directory_path || !file_path) {
    return false;
  }
  if (strlen(directory) >= strlen(file)) {
    return false;
  }
  if (strncmp(directory, file, strlen(directory)) != 0) {
    return false;
  }
  if (strncmp("/", &file[strlen(directory)], 1) != 0) {
    return false;
  }
  return true;
}
